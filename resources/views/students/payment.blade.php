...
<div class="p-6 lg:p-10">
    ...
    @if(!$isFullyPaid)
        <div class="bg-white border border-gray-100 rounded-[32px] p-6 lg:p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] mb-10 flex flex-col gap-4 lg:gap-6">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 w-full">
                <div class="text-left">
                    <h2 class="text-3xl lg:text-[32px] font-bold text-[#222222] tracking-tight">Total Tagihan: {{ $activeBill->total }}</h2>
    ...
    <div class="bg-white border border-gray-100 rounded-[32px] p-10 lg:p-14 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] mb-10 flex flex-col items-center justify-center text-center">
            ...
            <h2 class="text-2xl font-bold text-[#222222] mb-2">Semua Tagihan Sudah Lunas!</h2>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-[#F9FAFB] border-b border-gray-100">
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">Periode/Bulan</th>
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">Tanggal Pembayaran</th>
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">No. Transaksi</th>
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">Jumlah</th>
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">Status</th>
                        <th class="py-4 px-6 text-sm font-bold text-[#222222]">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @foreach($paymentHistory as $history)
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="py-4 px-6 text-sm font-semibold text-[#444444]">{{ $history->period }}</td>
                            <td class="py-4 px-6 text-sm font-semibold text-[#666666]">{{ $history->date }}</td>
                            <td class="py-4 px-6 text-sm font-medium text-[#666666]">{{ $history->no_trans }}</td>
                            <td class="py-4 px-6 text-sm font-semibold text-[#444444]">{{ $history->amount }}</td>
