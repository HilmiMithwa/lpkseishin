...
<div class="p-4 sm:p-6 lg:p-10">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 space-y-8">
            ...
            <div class="w-full bg-white border border-gray-100 rounded-[32px] shadow-sm overflow-hidden" x-data="discussionChat()" x-init="init()">
                {{-- Header --}}
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-400 font-semibold">Tanya jawab seputar mata pelajaran ini</p>
                        </div>
                    </div>
                </div>

                {{-- Messages Area --}}
                <div id="chat-messages" class="px-6 py-4 space-y-4 overflow-y-auto custom-scrollbar" style="max-height: 420px; min-height: 200px;">
                    {{-- Empty State --}}
                    <div x-show="messages.length === 0 && !loading" class="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p class="text-[10px] text-gray-300 mt-1">Mulai percakapan tentang mata pelajaran ini!</p>
                    </div>
...
<script>
    function discussionChat() {
        return {
            ...
            async fetchMessages() {
                try {
                    const res = await fetch(`/students/subjects/${this.mapelId}/discussions`);
            ...
            async pollMessages() {
                try {
                    const res = await fetch(`/students/subjects/${this.mapelId}/discussions/poll/${this.lastId}`);
                    const data = await res.json();
                    if (data.length > 0) {
