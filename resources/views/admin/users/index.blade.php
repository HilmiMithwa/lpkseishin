...
<div class="p-6 lg:p-8 space-y-6">
    ...
    <x-card class="overflow-hidden" padding="none">
        ...
        <div class="p-5 lg:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="relative w-full sm:w-64">
                ...
                <input type="text" class="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-xl leading-5 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#d62828]/20 focus:border-[#d62828] transition-all sm:text-sm font-medium text-slate-800" placeholder="Cari pengguna...">
            </div>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name=User+Testing&background=f3f4f6&color=d62828&bold=true" class="w-10 h-10 rounded-full object-cover">
                                <div>
                                    <p class="text-sm font-bold text-slate-800">User Testing</p>
                                    <p class="text-xs font-medium text-slate-500 mt-0.5">user@example.com</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm font-semibold text-slate-700">0812-3456-7890</p>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <p class="text-sm font-semibold text-slate-700">20 Mei 2026</p>
                        </td>
                        <td class="px-6 py-4 text-center" x-data>
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button x-on:click="$dispatch('open-edit-user-modal', { user: { role: 'siswa', name: 'User Testing', email: 'user@example.com', phone: '0812-3456-7890', school: 'SMA Negeri 1' } })" class="p-1.5 text-slate-400 hover:text-[#d62828] bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-rose-50 transition" title="Edit">
        ...
        <div class="p-4 border-t border-gray-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between text-sm gap-3">
            <span class="font-medium text-slate-500">Menampilkan <span class="font-bold text-slate-800">1</span> dari <span class="font-bold text-slate-800">1</span> pengguna</span>
    ...
    <x-modal name="add-user-modal" focusable maxWidth="2xl">
        <form method="POST" action="#" 
              x-data="{ role: 'siswa', isEdit: false, name: '', email: '', phone: '', school: '' }" 
              @open-add-user-modal.window="isEdit = false; role = 'siswa'; name = ''; email = ''; phone = ''; school = ''; $dispatch('open-modal', 'add-user-modal')"
              @open-edit-user-modal.window="isEdit = true; role = $event.detail.user.role; name = $event.detail.user.name; email = $event.detail.user.email; phone = $event.detail.user.phone; school = $event.detail.user.school; $dispatch('open-modal', 'add-user-modal')"
              x-on:submit.prevent="$dispatch('close'); $dispatch('show-toast', { message: isEdit ? 'Perubahan berhasil disimpan!' : 'Data pengguna berhasil disimpan!' })">
            @csrf
    ...
    <x-modal name="delete-user-modal" focusable maxWidth="md">
        <div class="p-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" x-on:click="$dispatch('close'); setTimeout(() => $dispatch('show-toast', { message: 'Pengguna berhasil dihapus!' }), 300)" class="flex-1 py-3.5 text-sm font-bold text-white bg-[#d62828] hover:bg-red-700 rounded-2xl shadow-md hover:shadow-lg transition-colors focus:outline-none focus:ring-2 focus:ring-[#d62828]/50">
                    Ya, Hapus
                </button>
            </div>
        </div>
