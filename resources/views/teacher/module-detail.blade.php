...
<div class="p-4 sm:p-6 lg:p-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <button x-data @click="$dispatch('open-edit-module-modal')" class="inline-flex items-center gap-2 bg-[#d62828] hover:bg-red-700 text-white font-bold py-2.5 px-5 rounded-lg text-xs sm:text-sm transition shadow-sm self-start">
            Edit Modul
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
{{-- Delete Item Modal (Sudah Terintegrasi Real Backend) --}}
<div x-data="{ open: false, itemId: null, itemName: '', itemType: '', deleteUrl: '', isLoading: false }"
    x-show="open"
    x-on:open-delete-item-modal.window="
        itemId = $event.detail.id; 
        itemName = $event.detail.name; 
        itemType = $event.detail.type; 
        // Bikin URL dinamis: jika tipe yang dihapus 'Materi', arahkan ke route destroy milik materi
        deleteUrl = $event.detail.type === 'Materi' 
            ? '/teacher/modules/{{ $currentModuleId }}/materials/' + $event.detail.id
            : '/teacher/modules/{{ $currentModuleId }}/tasks/' + $event.detail.id;
        open = true;
     "
    style="display: none;"
    ...
    <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
        <div x-show="open"
            ...
            {{-- PERBAIKAN UTAMA: Bungkus tombol aksi dengan Form Request asli Laravel --}}
            <form :action="deleteUrl" method="POST" @submit="isLoading = true" class="mt-8 sm:mt-6 sm:flex sm:flex-row-reverse gap-3">
                @csrf
                @method('DELETE') {{-- Spoofing method agar dibaca Route::delete oleh Laravel --}}
