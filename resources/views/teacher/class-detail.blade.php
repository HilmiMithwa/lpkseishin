...
<div class="p-4 sm:p-6 lg:p-10" x-data>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-1 flex flex-col gap-6 lg:gap-8">
            ...
            <div class="w-full bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden" x-data="discussionChat()" x-init="init()">
                {{-- Header --}}
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-[#222222]">Diskusi Kelas</h3>
                            <p class="text-[10px] text-gray-400 font-semibold">Tanya jawab seputar kelas ini</p>
                        </div>
                    </div>
                </div>

                {{-- Messages Area --}}
                <div id="teacher-chat-messages" class="px-6 py-4 space-y-4 overflow-y-auto custom-scrollbar" style="max-height: 420px; min-height: 200px;">
                    {{-- Empty State --}}
                    <div x-show="messages.length === 0 && !loading" class="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p class="text-xs font-bold text-gray-400">Belum ada diskusi</p>
                        <p class="text-[10px] text-gray-300 mt-1">Siswa belum memulai percakapan di materi ini.</p>
                    </div>

                    {{-- Loading --}}
                    <div x-show="loading" class="flex items-center justify-center py-10">
                        <svg class="animate-spin h-6 w-6 text-[#d62828]" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </div>

                    {{-- Message Bubbles --}}
                    <template x-for="msg in messages" :key="msg.id">
                        <div :class="msg.is_own ? 'flex justify-end' : 'flex justify-start'">
                            <div :class="msg.is_own ? 'max-w-[75%] order-last' : 'max-w-[75%]'" class="flex gap-2.5 items-end">
                                {{-- Avatar (other people only) --}}
                                <template x-if="!msg.is_own">
                                    <img :src="msg.user_photo ? msg.user_photo : 'https://ui-avatars.com/api/?name=' + encodeURIComponent(msg.user_name) + '&background=f3f4f6&color=d62828&bold=true&size=32'" class="w-7 h-7 rounded-full border border-gray-200 flex-shrink-0 object-cover" :alt="msg.user_name">
                                </template>
                                ...
                                <div>
                                    ...
                                    <template x-if="!msg.is_own">
                                        <div class="flex items-center gap-1.5 mb-1 px-1">
                                            <span class="text-[10px] font-bold text-[#444444]" x-text="msg.user_name"></span>
                                            <template x-if="msg.is_guru">
                                                <span class="bg-[#d62828] text-white text-[8px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider">Sensei</span>
                                            </template>
                                        </div>
                                    </template>
                                    ...
                                    <div :class="msg.is_own 
                                        ? 'bg-[#d62828] text-white rounded-2xl rounded-br-md' 
                                        : (msg.is_guru ? 'bg-red-50 border border-red-100 text-[#222222] rounded-2xl rounded-bl-md' : 'bg-gray-50 border border-gray-100 text-[#222222] rounded-2xl rounded-bl-md')" 
                                        class="px-4 py-2.5 text-xs font-medium leading-relaxed">
                                        <p x-text="msg.message"></p>
                                    </div>

                                    {{-- Time --}}
                                    <div :class="msg.is_own ? 'text-right' : 'text-left'" class="mt-1 px-1">
                                        <span class="text-[9px] text-gray-300 font-semibold" x-text="msg.created_at"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                    <form @submit.prevent="sendMessage()" class="flex items-center gap-3">
                        <div class="flex-1 relative">
                            <input type="text" x-model="newMessage" placeholder="Balas diskusi siswa..." 
                                class="w-full rounded-xl border-gray-200 bg-white focus:border-[#d62828] focus:ring focus:ring-red-200 transition text-xs py-3 pr-4 pl-4 font-medium"
                                maxlength="1000"
                                @keydown.enter.prevent="sendMessage()">
                        </div>
                        <button type="submit" :disabled="sending || !newMessage.trim()" 
                            class="w-10 h-10 bg-[#d62828] hover:bg-red-700 text-white rounded-xl flex items-center justify-center transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0">
                            <svg x-show="!sending" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
...
<script>
    function discussionChat() {
        return {
            ...
            async fetchMessages() {
                try {
                    const res = await fetch(`/teacher/subjects/${this.mapelId}/discussions`);
            ...
            async pollMessages() {
                try {
                    const res = await fetch(`/teacher/subjects/${this.mapelId}/discussions/poll/${this.lastId}`);
                    const data = await res.json();
                    if (data.length > 0) {
